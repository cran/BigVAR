...
MatrixXd VARXConsInternal(MatrixXd Y, MatrixXd X, const int k, const int p, const int m,  int s,bool oos=false,bool contemp=false)
{
	// int T=Y.rows();
...
List ICX(NumericMatrix Y1, NumericMatrix X1, double k, int pmax,int smax,double m,std::string pen,int h=1)
{
			...
			for(int j=0;j<=smax;++j)    
				{
					if((int) (k*i+j*m)>T)
						...
						MatrixXd Z;
							if(m>0){
								Z=VARXConsInternal(Y,X,k,i,m,j);
							}else{
								Z=ZmatF(Y,i,k);
							}
	...
	MatrixXd BResF;
	MatrixXd ZF=VARXConsInternal(Y,X,k,phat,m,shat);
